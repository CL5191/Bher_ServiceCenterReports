...
param(
    ...
    [string]$OutputName = "Behr-SolutionsCenterReport",
...
if ([string]::IsNullOrWhiteSpace($QueueMapPath)) {
    $QueueMapPath = Join-Path $repoRoot "Config\queues-solutions-center.json"
}
...
try {
    Write-ReportLog -LogFile $run.LogFile -Message "Solutions Center on-demand report run started. RunId=$($run.RunId)"
    ...
    Write-ReportLog -LogFile $run.LogFile -Level ERROR -Message ("Solutions Center on-demand report failed: {0}" -f $_.Exception.Message)
    throw
}
finally {
