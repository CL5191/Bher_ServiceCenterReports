...
try {
    Write-ReportLog -LogFile $run.LogFile -Message "Solutions Center monthly report run started. RunId=$($run.RunId)"
    ...
    $outputName = "Behr-SolutionsCenterMonthly"
    ...
    Write-ReportLog -LogFile $run.LogFile -Level ERROR -Message ("Solutions Center monthly report failed: {0}" -f $_.Exception.Message)
    throw
}
finally {
